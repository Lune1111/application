<template>
	<view class="wrap" style="padding-top:50rpx;">
		...
		<view class="remind-text">
			...
			<u-button v-if="codeDisabled" class="button" @tap="getCode('/pages/sys/login/registration?phoneNo='+this.phoneNo)">{{tips}}</u-button>
		</view>
	</view>
</template>
<script>
      ...
      finish(e){
        this.$u.api.sendSms({yzm:e+this.phoneNo}).then(res=>{
          if(res.code==200) {
            setTimeout(() => {
              uni.reLaunch({
                url: '/pages/sys/login/registration?phoneNo='+this.phoneNo
              });
            }, 500);
          }else{
