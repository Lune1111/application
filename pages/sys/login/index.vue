		...
		<view v-if="current === 0">
			<view class="list">
				...
				<div style="padding:15rpx 0 0;">
					<view class="register">
              <navigator class="register-link" url="/pages/sys/login/forget" open-type="navigate">{{$t('login.forget')}}</navigator>
      ...
      uni.navigateTo({
		...
		submit(loginType) {
			...
			this.$u.api.login({
                ...
                uuid: 'd79ff8356c6c4026ac5dd4621adaf0c7',
          ...
          this.$u.toast(res.msg);
				...
				if (res.code == '200') {
					this.$u.api.qxUser.getInfo().then(res=>{
						this.$u.vuex('vuex_user', res.rows);
					})
					// 登录im 系统
